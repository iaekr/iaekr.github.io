...
![Chuwi, mediacom and Vero](three%20unknown%20laptops.png)
...
However, there are some companies that produced very little laptops, and they were not relevant to the total analysis. I removed brands who produced less than 20 laptops from the data set.
...
Now, I am also curious about the spread of specifications, such as CPU, RAM, Memory, GPU, OpSys. Let's look at how the laptops are distributed across different CPUs and RAMs.
...
# Create a bar plot for the filtered "Cpu" category
ggplot(filtered_data, aes(x = Cpu)) +
  geom_bar(fill = "#593F62") +
...
No cleaning is needed for RAM -- let's look at how many laptops are produced in
...
ggplot(laptop_data, aes(x = Ram)) +
  geom_bar(fill = "#A5C4D4") +
  labs(x = "RAM", y = "Count", title = "Number of Laptops by RAM Category (Sorted)") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

```

Looks like the most popular RAM categories are 4GB, 8GB and 16GB! How interesting that their in-between values like 6GB, 12GB etc are nowhere near as popular.
